...
FILE NAME: problem_%i_s2012.csv e.g. problem_330_s2012.csv
...
"""
    ...
    for problem_id in problemList:
        ...
        if certified:
            ...
            filename = "problem_%i_s2012.csv"%(problem_id)
        f=open(filename,'wb')
        f.close()
...
"""%(str(user_id),first_time,last_time)
            ...
            with open(filename,'a') as csvfile:
			    ...
			    writer.writerow(list(all_responses[pIndex])[:2]+[all_responses[pIndex][4],int(all_responses[pIndex][2]),answerID])
                    	...
                    	if rIndex != 0: #only writes observed events with different consecutive url_id or user_id
                    	    if all_resources[rIndex-1][0] != all_resources[rIndex][0] or all_resources[rIndex-1][2] != all_resources[rIndex][2]:
...
if __name__ == "__main__":
    ...
    cursor = db.cursor()
    problemList = []
    for x in range(62,100): #change this to vary which problems to generate
