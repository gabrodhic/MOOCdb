...
FILE NAME: problem_%i_s2013.csv e.g. problem_330_s2013.csv
...
"""
    ...
    for problem_id in problemList:
        ...
        if certified:
            ...
            filename = "problem_%i_s2013.csv"%(problem_id)
        f=open(filename,'wb')
        f.close()
...
if __name__ == "__main__":
    ...
    port = 3306
    user = "kevin"
    database = "moocdb_6002x_spring_2013"
    pw = "cosmos5"

    db = MySQLdb.connect(host=host, port=port, user=user, db=database, passwd=pw)
    cursor = db.cursor()
    problemList = []
    for x in range(25,26): #change this to vary which problems to generate
