...
def main():
    ...
    SELECT 
         ...
         COUNT(*) AS `Number of days between the last submission and the deadline for all labs`
    ...
    WHERE submissions.submission_attempt_number = ( -- We just want the last attempt
        ...
        AND problems.problem_type_id = 3 -- = 3 means Lab
     GROUP BY day
     ORDER BY day ASC 
    ;
    ;
    ...
    options.set_chart_title("Number of days between the last submission and the deadline for all labs")
    options.set_height(500)
    options.set_width(1200)
    options.set_page_title("Number of days between the last submission and the deadline for all labs")
    ...
    options.set_output_file("./output/days_last_submission_before_deadline_lab.html")
